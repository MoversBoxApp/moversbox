...
/*Google places autocomplete code in custom-form-wizard.js file*/
...
function finditem(array, it){
  var index = 15;//Just a random value
...
function SetSummary(){
  ...
  var cargo_quantity = document.getElementsByName("quantity[]");
...
for (i = 0; i < firstname.length; i++){
  contact = contact + '<li><strong>Contact #' + (i+1) + ' :</strong>' + firstname[i].value + ' ' + lastname[i].value + '</li>' +
...
for (k = 0; k < dropoff_address.length; k++){
  ...
  cargo.forEach((item, i) => {
    $cargo_smry = $cargo_smry + cargo_quantity[i].value + ' ' + item.name + ', ';
  })
  $(summary).append(
    ...
    '<li><strong>Estimated Time :</strong> ' + estimated_time + ' hours</li>' +
